...
- 한국 뉴스 3개:
  - 경제 뉴스 2개
  - 주식과 연결된 흥미 뉴스 1개 (게임, AI, 제품, 트렌드 등 반드시 기업/주식과 연결)

- 세계 뉴스 3개:
  - 경제 뉴스 2개
  - 주식과 연결된 흥미 뉴스 1개 (반드시 기업/주식 연결)

- 모든 뉴스는 반드시 "주식(stockIds)"과 자연스럽게 연결되어야 함
...
- 반드시 “충격 / 변화 / 궁금증”이 느껴지게 작성
- “~쇼크”, “~비상”, “~흔든다”, “왜 ~일까?” 같은 표현 활용
- 핵심 경제 키워드(금리, 물가, 반도체 등) 포함
- 20자 이내
...
- 한 문장은 1~2개의 정보만 담고 자연스럽게 이어질 것
...
- 반드시 경제 용어 포함:
  ...
  인플레이션(물가가 계속 오르는 현상)
  환율(두 나라 돈의 교환 비율)
  AI(인공지능, 사람처럼 생각하는 기술)

- 경제 용어는 처음 등장 시 괄호 설명 필수
- 이후 재사용 시 설명 생략

- 설명은 자연스럽게 이어지도록 작성 (나열 금지)

- 중간에 1~2번 정도 가벼운 궁금증/놀람 표현 포함
  (예: “왜 이런 일이 생겼을까요?”)

- 마지막 문장은 반드시
  👉 "그래서 우리 생활에 어떤 영향이 있는지"로 마무리
...
- KR 뉴스 id:
  news_YYYY-MM-DD_kr_1 ~ 3

- US 뉴스 id:
  news_YYYY-MM-DD_us_1 ~ 3

- 뉴스 총 6개 작성
- 각 뉴스마다 퀴즈 1개

- answerIndex는 0,1,2 랜덤

- stockIds:
  - 한국 기업 → 한글
  - 미국 기업 → 영어

- createdAt, date는 동일한 오늘 날짜`}
          </PromptBox>
